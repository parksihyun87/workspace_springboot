    ...
    public class Controller {
        ...
        @PutMapping(value = "/product-update")//해당 id 필요하고 이걸로 내용 입력시 수정
        public ProductDTO updateProduct(@RequestBody ProductDTO productDTO){
            Optional<Product> updateProduct= this.productRepository.findById(productDTO.getId());
            if(updateProduct.isPresent()){
                Product completeProduct= updateProduct.get();
                completeProduct.setPrice(productDTO.getPrice());
                Product saveProduct =this.productRepository.save(completeProduct);
                ProductDTO updateProductDTO = new ProductDTO();
                updateProductDTO.setTitle(saveProduct.getTitle());
                updateProductDTO.setPrice(saveProduct.getPrice());
                updateProductDTO.setImgSrc(saveProduct.getImgSrc());
                updateProductDTO.setId(saveProduct.getId());
