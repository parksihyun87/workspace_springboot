...
public class JwtLoginFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
        ...
        // 성공시 토큰 만드는 2가지가 중요함.
        String token = this.jwtUtil.createToken(username,role,60*10*1000L);//(유저네임, 롤, (유효기간,L-2가지))3가지를 챙겨서 넣어줘야 함
        response.addHeader("Authorization","Bearer "+token);// 헤더에 쓰는 메서드, 여기서 헤더에 토큰이 들어간다.
